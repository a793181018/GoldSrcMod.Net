...
namespace AmxModx.Wrappers.Engine
{
    ...
    public static class EngineManager
    {
        ...
        public static TraceResult TraceLine(EngineBridge.Vector3 start, EngineBridge.Vector3 end, int ignoreEntity = 0)
        {
            EngineBridge.TraceResultInfo result;
            int success = EngineBridge.Engine_TraceLine(start, end, ignoreEntity, out result);
            
            return new TraceResult(result, success == 1);
        }
        ...
        /// <returns>追踪结果</returns>
        public static TraceResult TraceHull(EngineBridge.Vector3 start, EngineBridge.Vector3 end, int hullType, int ignoreEntity = 0)
        {
            EngineBridge.TraceResultInfo result;
            int success = EngineBridge.Engine_TraceHull(start, end, hullType, ignoreEntity, out result);
        ...
        public static EngineBridge.Vector3 GetEntityOrigin(int entityId)
        {
            if (!IsValidEntity(entityId))
                return new EngineBridge.Vector3(0, 0, 0);

            if (EngineBridgeExtensions.GetEntityOrigin(entityId, out var origin))
                return origin;
        ...
        public static EngineBridge.Vector3 GetEntityAngles(int entityId)
        {
            if (!IsValidEntity(entityId))
                return new EngineBridge.Vector3(0, 0, 0);

            if (EngineBridgeExtensions.GetEntityAngles(entityId, out var angles))
                return angles;
        ...
        public static EngineBridge.Vector3 GetEntityVelocity(int entityId)
        {
            if (!IsValidEntity(entityId))
                return new EngineBridge.Vector3(0, 0, 0);

            if (EngineBridgeExtensions.GetEntityVelocity(entityId, out var velocity))
                return velocity;
    ...
    {
        private readonly EngineBridge.TraceResultInfo _info;
        private readonly bool _success;

        public TraceResult(EngineBridge.TraceResultInfo info, bool success)
        {
            _info = info;
            _success = success;
        }
        ...
        /// 结束位置
        /// </summary>
        public EngineBridge.Vector3 EndPosition => _info.EndPos != null ? 
            new EngineBridge.Vector3(_info.EndPos[0], _info.EndPos[1], _info.EndPos[2]) : 
            new EngineBridge.Vector3();
        ...
        /// 平面法线
        /// </summary>
        public EngineBridge.Vector3 PlaneNormal => _info.PlaneNormal != null ? 
            new EngineBridge.Vector3(_info.PlaneNormal[0], _info.PlaneNormal[1], _info.PlaneNormal[2]) : 
            new EngineBridge.Vector3();
