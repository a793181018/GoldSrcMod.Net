...
namespace AmxModx.Wrappers.Communication
{
    ...
    public static class ForwardManager
    {
        ...
        private static readonly Dictionary<string, int> _forwardIds = new Dictionary<string, int>();
        ...
        public static void RegisterForward(string forwardName, ForwardCallback callback, params int[] paramsTypes)
        {
            ...
            // 注册到AMX Mod X转发系统
            int forwardId = Forward.AmxModx_Bridge_CreateMultiForward(forwardName, Forward.ForwardExecType.Ignore, paramsTypes.Length, paramsTypes);
            _forwardIds[forwardName] = forwardId;
        }
        ...
        public static void UnregisterForward(string forwardName)
        {
            ...
            if (_forwardIds.TryGetValue(forwardName, out int forwardId))
            {
                Forward.AmxModx_Bridge_DestroyForward(forwardId);
                _forwardIds.Remove(forwardName);
            }
        }
        ...
        public static bool ForwardExists(string forwardName)
        {
            ...
            if (_forwardIds.TryGetValue(forwardName, out int forwardId))
            {
                return Forward.AmxModx_Bridge_GetForwardParamCount(forwardId) >= 0;
            }
        ...
        public static int GetForwardParamCount(string forwardName)
        {
            ...
            if (_forwardIds.TryGetValue(forwardName, out int forwardId))
            {
                return Forward.AmxModx_Bridge_GetForwardParamCount(forwardId);
            }
        ...
        public static int GetForwardParamType(string forwardName, int paramIndex)
        {
            ...
            if (_forwardIds.TryGetValue(forwardName, out int forwardId))
            {
                return Forward.AmxModx_Bridge_GetForwardParamType(forwardId, paramIndex);
            }
        ...
        {
            foreach (var forwardId in _forwardIds.Values)
            {
                Forward.AmxModx_Bridge_DestroyForward(forwardId);
            }
