...
namespace AmxModx.Bridge.Engine
{
    ...
    public static class EngineBridgeExtensions
    {
        ...
        public static bool GetEntityOrigin(int entityId, out Vector3 origin)
        {
            float[] originArray = new float[3];
            bool result = Engine_GetEntityOrigin(entityId, originArray) != 0;
            if (result)
            {
                origin = new Vector3(originArray[0], originArray[1], originArray[2]);
            }
            else
            {
        ...
        /// <param name="angles">返回的角度</param>
        /// <returns>成功返回true，失败返回false</returns>
        public static bool GetEntityAngles(int entityId, out Vector3 angles)
        {
            float[] anglesArray = new float[3];
            bool result = Engine_GetEntityAngles(entityId, anglesArray) != 0;
            if (result)
            {
                angles = new Vector3(anglesArray[0], anglesArray[1], anglesArray[2]);
            }
            else
            {
        ...
        /// <param name="velocity">返回的速度</param>
        /// <returns>成功返回true，失败返回false</returns>
        public static bool GetEntityVelocity(int entityId, out Vector3 velocity)
        {
            float[] velocityArray = new float[3];
            bool result = Engine_GetEntityVelocity(entityId, velocityArray) != 0;
            if (result)
            {
                velocity = new Vector3(velocityArray[0], velocityArray[1], velocityArray[2]);
            }
            else
            {
